...
use log::info;
use nom::{
    bytes::complete::tag, character::complete::multispace1, sequence::tuple, IResult, Parser,
};
...
fn parse_header(input: &str) -> IResult<&str, Header> {
    info!("input: {}", input);
    let (input, _) = tuple((tag("p"), multispace1, tag("cnf"), multispace1)).parse(input)?;
    info!("input: {}", input);
...
pub fn parse(input: &str) -> Result<Vec<Clause>> {
    ...
    let clauses = lines
        ...
        .map(|line| {
            parse_clause(line)
                .map(|(_, clause)| clause)
                .map_err(|_| anyhow!("invalid clause"))
        })
